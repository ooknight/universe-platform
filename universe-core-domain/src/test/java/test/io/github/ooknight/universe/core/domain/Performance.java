...
public class Performance {
    ...
    public void test() {
        ...
        for (int i = 0; i < count; i++) {
            assertEquals("1", SampleState1.YES.code());
            assertEquals("2", SampleState1.NO.code());
        }
        long t1 = stopwatch.elapsed(TimeUnit.MILLISECONDS);
        //
        stopwatch.reset().start();
        for (int i = 0; i < count; i++) {
            assertEquals("1", SampleState2.YES.code());
            assertEquals("2", SampleState2.NO.code());
        }
        long t2 = stopwatch.elapsed(TimeUnit.MILLISECONDS);
        //
        stopwatch.reset().start();
        for (int i = 0; i < count; i++) {
            assertEquals("1", SampleState3.YES.code());
            assertEquals("2", SampleState3.NO.code());
        }
